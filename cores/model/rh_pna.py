...
from cores.model.modules.FeatureExtractionBlock import FeatureExtractionBlock_Conv, FeatureExtractionBlock_PNA, FeatureExtractionBlock_PNAV2

from cores.model.modules.FeatureAggregationBlock import FeatureAggregationBlock_Conv, FeatureAggregationBlock_PNA, FeatureAggregationBlock_PNAV2, FeatureAggregationBlock_PNAV3
...
class UNet(nn.Module):
    def __init__(self, 
        ...
        # self.FEB_Conv_128 = FeatureExtractionBlock_Conv(
        #     geo_feat_channels
        # )

        # self.FEB_Conv_64 = FeatureExtractionBlock_Conv(
        #     geo_feat_channels
        # )

        # self.FEB_Conv_32 = FeatureExtractionBlock_Conv(
        #     geo_feat_channels
        # )
        ...
        self.FEB_PNA_128 = FeatureExtractionBlock_PNAV2(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
        )

        self.FEB_PNA_64 = FeatureExtractionBlock_PNAV2(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
        )

        self.FEB_PNA_32 = FeatureExtractionBlock_PNAV2(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
        )
        ...
        # PNA Branch
        self.FAB_PNA_32 = FeatureAggregationBlock_PNAV2(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
            kernel_size=kernel_size[0],
        ...
        self.FAB_PNA_64 = FeatureAggregationBlock_PNAV2(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
            kernel_size=kernel_size[1],
        ...
        self.FAB_PNA_128 = FeatureAggregationBlock_PNAV2(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
            kernel_size=kernel_size[2],
        ...
        self.FAB_Conv_256 = FeatureAggregationBlock_Conv(
            geo_feat_channels, 
        )

        self.pred_head_pna = PredHeaders(
            geo_feat_channels=geo_feat_channels,
            num_class=num_class
        )
    ...
    def forward(self, x):  # [b, geo_feat_channels, X, Y, Z]   
        ...
        # x, skip_128 = self.FEB_Conv_128(x) # skip2: ([1, 64, 128, 128, 16]) / x: ([1, 64, 64, 64, 8])  
        # x, skip_64 = self.FEB_Conv_64(x) # skip3: ([1, 64, 64, 64, 8]) / x: ([1, 64, 32, 32, 4])
        # x, skip_32 = self.FEB_Conv_32(x) # skip4: ([1, 64, 32, 32, 4]) / x: ([1, 64, 16, 16, 2])

        
        x_16 = self.bottleneck(x) # x: ([1, 64, 16, 16, 2])
        
        # PNA
        x_pna_32 = self.FAB_PNA_32(skip_32, x_16)  # x: ([1, 64, 32, 32, 4])
        x_pna_64 = self.FAB_PNA_64(skip_64, x_pna_32)  # x: ([1, 64, 64, 64, 8])    
        x_pna_128 = self.FAB_PNA_128(skip_128, x_pna_64)  # x: ([1, 64, 128, 128, 16])      
        x_pna_256 = self.FAB_Conv_256(x_pna_128, skip_256)  # x: ([1, 64, 256, 256, 32])
        
        # pred
        x_pna_32, x_pna_64, x_pna_128, x_pna_256 = self.pred_head_pna(x_pna_32, x_pna_64, x_pna_128, x_pna_256)

        return [x_pna_32, x_pna_64, x_pna_128, x_pna_256]
...
class UNetV3(nn.Module):
    def __init__(self, 
        ...
        # PNA Branch
        self.FAB_PNA_32 = FeatureAggregationBlock_PNAV3(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
            kernel_size=kernel_size[0],
        ...
        self.FAB_PNA_64 = FeatureAggregationBlock_PNAV3(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
            kernel_size=kernel_size[1],
        ...
        self.FAB_PNA_128 = FeatureAggregationBlock_PNAV3(
            geo_feat_channels=geo_feat_channels,
            ffn_cfg=ffn_cfg,
            num_heads=num_heads,
            kernel_size=kernel_size[2],
...
from cores.model.pna_modules.ProgressiveNeighborhoodAggregation import ProgressiveNeighborhoodAggregation as PNA
...
class RefHead_PNA(nn.Module):
    def __init__(
        ...
        super(RefHead_PNA, self).__init__()
