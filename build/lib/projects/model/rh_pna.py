...
from .pna import ProgressiveNeighborhoodAggregation as PNA

class ConvBlock(nn.Module):
    def __init__(self, geo_feat_channels, conv_version, padding_mode='replicate', stride=(1, 1, 1), kernel_size = (5, 5, 3), padding = (2, 2, 1)):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, geo_feat_channels, conv_version, z_down):
        super().__init__()
        self.z_down = z_down
        
        self.convblock = ConvBlock(geo_feat_channels=geo_feat_channels, conv_version='v1')
...
class Decoder(nn.Module):
    def __init__(self, geo_feat_channels, skip_version, conv_version):
        super().__init__()
        
        self.convblock = ResConvBlock(geo_feat_channels=geo_feat_channels, skip_version=skip_version, conv_version=conv_version)
...
class UNet(nn.Module):
    ...
    def forward(self, x):  # [b, geo_feat_channels, X, Y, Z]   
        ...
        x4 = self.pna_block_4(skip4, x)  # x: ([1, 64, 32, 32, 4])
        
        x3 = self.pna_block_3(skip3, x4)  # x: ([1, 64, 64, 64, 8]) 
