...
class DeformableCrossAttention2D(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.dropout = nn.Dropout(dropout)
        self.to_q = nn.Conv2d(dim, inner_dim, 1, groups = offset_groups if group_queries else 1, bias = False)
        self.to_k = nn.Conv2d(kv_dim, inner_dim, 1, groups = offset_groups if group_key_values else 1, bias = False)
        self.to_v = nn.Conv2d(kv_dim, inner_dim, 1, groups = offset_groups if group_key_values else 1, bias = False)
        self.to_out = nn.Conv2d(inner_dim, dim, 1)

    def forward(self, x, kv, return_vgrid = False):
        """
        b - batch
        h - heads
        x - height
...
if __name__ == '__main__':
    # python /u/home/caoh/projects/MA_Jiachen/3DPNA/projects/model/dfa/DFA2D.py
    voxel = torch.randn(1, 32, 128, 32).cuda()
    img = torch.randn(1, 512, 128, 32).cuda()


    d = DeformableCrossAttention2D(
        dim=32,
        kv_dim=512,
        dim_head = 4,
        heads = 8,
