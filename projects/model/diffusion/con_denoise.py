...
class Asymmetric_Residual_Block(nn.Module):
    def __init__(self, in_filters, out_filters, time_filters=32*4):
        ...
        self.GroupNorm = nn.GroupNorm(10, in_filters)
        self.bn0 = nn.GroupNorm(10, out_filters)
        self.bn0_2 = nn.GroupNorm(10, out_filters)
        self.bn1 = nn.GroupNorm(10, out_filters)
        self.bn2 = nn.GroupNorm(10, out_filters)
...
class DDCM(nn.Module):
    def __init__(self, in_filters, out_filters, kernel_size=(3, 3, 3), stride=1):
        ...
        self.bn0 = nn.GroupNorm(10, out_filters)
        self.bn0_2 = nn.GroupNorm(10, out_filters)
        self.bn0_3 = nn.GroupNorm(10, out_filters)
...
class Attention_torch(nn.Module):
    def __init__(self, embed_dims, num_heads=4, bias=True):
        super().__init__()
        # Generate QKV
        ...
        #out
        self.to_out = conv1x1(embed_dims, embed_dims, stride=1)
    ...
    def forward(self, x):
        ...
        k = rearrange(k, 'b c h w z -> b (h w z) c')
        v = rearrange(v, 'b c h w z -> b (h w z) c')

        attn_output, attn_output_weights = self.mha(q, k, v)

        out = rearrange(attn_output, 'b (h w z) c -> b c h w z', h=h, w=w, z=z)
...
class UpBlock(nn.Module):
    def __init__(self, in_filters, out_filters, height_pooling, time_filters=32*4):
        ...
        self.trans_bn = nn.GroupNorm(10, in_filters)
        self.bn1 = nn.GroupNorm(10, out_filters)
        self.bn2 = nn.GroupNorm(10, out_filters)
        self.bn3 = nn.GroupNorm(10, out_filters)
...
class Denoise(nn.Module):
    def __init__(self, num_class = 20, init_size=16, discrete=True):
        ...
        self.conv_in = nn.Conv3d(init_size * 2, init_size, kernel_size=1, stride=1)
        ...
        self.downBlock4 = DownBlock(8 * init_size, 8 * init_size, height_pooling=False, time_filters=init_size*4)
        
        self.midBlock1 = Asymmetric_Residual_Block(8 * init_size, 8 * init_size, time_filters=init_size*4)
        self.attention = Attention_torch(8 * init_size, 4)
        self.midBlock2 = Asymmetric_Residual_Block(8 * init_size, 8 * init_size, time_filters=init_size*4)

        self.upBlock4 = UpBlock(8 * init_size, 8 * init_size, height_pooling=False, time_filters=init_size*4)
