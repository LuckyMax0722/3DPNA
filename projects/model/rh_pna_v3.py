...
class UNet(nn.Module):
    def __init__(self, 
        ...
        self.encoder_block_128 = Encoder(
            geo_feat_channels
        )

        self.encoder_block_64 = Encoder(
            geo_feat_channels
        )

        self.encoder_block_32 = Encoder(
            geo_feat_channels
        )
    ...
    def forward(self, x):  # [b, geo_feat_channels, X, Y, Z]   
        ...
        x, skip_128 = self.encoder_block_128(x) # skip2: ([1, 64, 128, 128, 16]) / x: ([1, 64, 64, 64, 8])
        
        x, skip_64 = self.encoder_block_64(x) # skip3: ([1, 64, 64, 64, 8]) / x: ([1, 64, 32, 32, 4])
        
        x, skip_32 = self.encoder_block_32(x) # skip4: ([1, 64, 32, 32, 4]) / x: ([1, 64, 16, 16, 2])
        ...
        x_256 = self.decoder_block_256(x_128, skip_256)  # x: ([1, 64, 256, 256, 32])
...
class RefHead_PNA_V3(nn.Module):
    def __init__(
        self,
        num_class=None,
        ...
        super(RefHead_PNA_V3, self).__init__()
